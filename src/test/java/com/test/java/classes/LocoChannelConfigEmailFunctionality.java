...
import com.test.java.utilities.LocoChannelConfigEmailWorkflow;
...
public class LocoChannelConfigEmailFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void ChannelCofigurationEmail(String user) throws Exception	 {

		try {
			
			CommonFunctions.login(driver, user);
			
			LocoChannelConfigEmailWorkflow.goingAccountSession(driver);
			LocoChannelConfigEmailWorkflow.ChannelConfigurationEmailWorkFlow(driver);
