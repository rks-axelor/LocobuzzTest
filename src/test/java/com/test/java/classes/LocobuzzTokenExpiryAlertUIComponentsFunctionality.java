...
import com.test.java.utilities.LocobuzzTokenExpiryAlertUIComponents;
...
public class LocobuzzTokenExpiryAlertUIComponentsFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void TokenExpiryAlert(String user) throws Exception	 {

		try {
			CommonFunctions.login(driver, user);
			
			LocobuzzTokenExpiryAlertUIComponents.goingAccountSession(driver);
			LocobuzzTokenExpiryAlertUIComponents.verifyingTokenExpiryAlertComponents(driver);
