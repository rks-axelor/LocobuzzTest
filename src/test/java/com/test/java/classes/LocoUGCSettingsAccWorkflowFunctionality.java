...
import com.test.java.utilities.LocoUGCSettingsAccWorkflowProcess;
...
public class LocoUGCSettingsAccWorkflowFunctionality {
...
@Test
@Parameters({ "user" })
public void LocoUGCSettingAccWorkflowProcess(String user) throws Exception	 {

	try {
		
		CommonFunctions.login(driver, user);
		
		LocoUGCSettingsAccWorkflowProcess.goingAccountSession(driver);
		LocoUGCSettingsAccWorkflowProcess.AgentQueueManagementUIVerificationprocess(driver);
