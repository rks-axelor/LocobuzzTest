...
import com.test.java.utilities.LocobuzzActionablenonactionableUIProcess;
...
public class LocobuzzActionablenonactionableUIVerFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void LogicalGrouping(String user) throws Exception	 {

		try {
			
			CommonFunctions.login(driver, user);
			
			LocobuzzActionablenonactionableUIProcess.goingAccountSession(driver);
			LocobuzzActionablenonactionableUIProcess.verifyingAutoCannedUIComponents(driver);
