...
import com.test.java.utilities.LocoInfluencerConfigTicketSectionWorkflow;
...
public class LocoInfluencerConfigTicketSectionFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void InflunenserRules(String user) throws Exception	 {

		try {
			CommonFunctions.login(driver, user);
			
			LocoInfluencerConfigTicketSectionWorkflow.goingAccountSession(driver);
			LocoInfluencerConfigTicketSectionWorkflow.verifyingInfluenserConfigurationFunction(driver);
