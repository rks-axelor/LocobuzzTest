...
public class LocobuzzMessagingWindowSettingsUIComponentsFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void MessagingWindowSettings(String user) throws Exception	 {

		try {
			CommonFunctions.login(driver, user);
			
			LocobuzzMessagingWindowSettingsUIComponents.goingAccountSession(driver);
			LocobuzzMessagingWindowSettingsUIComponents.verifyingMessagingWindowSettingsComponents(driver);
