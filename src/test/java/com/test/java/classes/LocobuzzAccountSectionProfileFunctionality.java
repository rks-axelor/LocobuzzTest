...
import com.test.java.utilities.LocobuzzAccountSectionProfileInformation;
...
public class LocobuzzAccountSectionProfileFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void Profile(String user) throws Exception	 {

		try {
			CommonFunctions.login(driver, user);
			
			LocobuzzAccountSectionProfileInformation.goingAccountSession(driver);
			LocobuzzAccountSectionProfileInformation.verifyingAccountComponents(driver);
			LocobuzzAccountSectionProfileInformation.verfyingProfileComponents(driver);
