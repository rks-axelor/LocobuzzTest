...
import com.test.java.utilities.LocoPublishWaterMarksAccountWorkFlow;
...
public class LocoPublishWaterMarksAccountFunctionality {
...
@Test
@Parameters({ "user" })
public void PublishWatermarks(String user) throws Exception	 {
try {
			
			CommonFunctions.login(driver, user);
			LocoPublishWaterMarksAccountWorkFlow.goingAccountSession(driver);
			LocoPublishWaterMarksAccountWorkFlow.verifyingPublishApproverUIComponents(driver);
