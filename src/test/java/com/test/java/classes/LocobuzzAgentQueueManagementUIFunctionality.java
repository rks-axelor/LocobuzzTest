...
import com.test.java.utilities.LocobuzzAgentQueueManagementUIProcess;
import com.test.java.utilities.LocobuzzCompetitorsWorkflowProcess;
...
public class LocobuzzAgentQueueManagementUIFunctionality {
...
@Test
@Parameters({ "user" })
public void LogicalGroupingWorkFlow(String user) throws Exception	 {

	try {
		
		CommonFunctions.login(driver, user);
		
		LocobuzzAgentQueueManagementUIProcess.goingAccountSession(driver);
		LocobuzzAgentQueueManagementUIProcess.AgentQueueManagementUIVerificationprocess(driver);
