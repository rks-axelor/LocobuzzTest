...
import com.test.java.utilities.LocobuzzMessagingWindowSettingsLayout2Workflow;
...
public class LocobuzzMessagingWindowSettingsLayout2WorkflowFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void MessagingWindowSettingsLayout2Flow(String user) throws Exception	 {

		try {
			CommonFunctions.login(driver, user);
			
			LocobuzzMessagingWindowSettingsLayout2Workflow.goingAccountSession(driver);
			LocobuzzMessagingWindowSettingsLayout2Workflow.verifyingMessagingWindowSettingsWorkflow(driver);
