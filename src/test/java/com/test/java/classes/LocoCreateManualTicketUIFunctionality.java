...
import com.test.java.utilities.LococreateManualTicketUIWorkflow;
...
public class LocoCreateManualTicketUIFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void CreateManualticket(String user) throws Exception	 {

		try {
			
			CommonFunctions.login(driver, user);
			
			LococreateManualTicketUIWorkflow.goingAccountSession(driver);
			LococreateManualTicketUIWorkflow.CreateManualticketFunctionality(driver);
