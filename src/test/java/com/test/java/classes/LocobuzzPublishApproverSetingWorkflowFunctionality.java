...
import com.test.java.utilities.LocobuzzPublishApproverSettingWorkflow;
...
public class LocobuzzPublishApproverSetingWorkflowFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void publishApprover(String user) throws Exception	 {

		try {
			
			CommonFunctions.login(driver, user);
			
			LocobuzzPublishApproverSettingWorkflow.goingAccountSession(driver);
			LocobuzzPublishApproverSettingWorkflow.PublishApproverworkprocess(driver);
