...
import com.test.java.utilities.LocobuzzLanguageFilterUIComponents;
...
public class LocobuzzLanguageFilterWorkFlowFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void LanguageFilter(String user) throws Exception	 {

		try {
			CommonFunctions.login(driver, user);
			
			LocobuzzLanguageFilterWorkProcess.goingAccountSession(driver);
			LocobuzzLanguageFilterWorkProcess.languageFilterProcess(driver);
