...
import com.test.java.utilities.LocobuzzCompetitorsUIVerificationProcess;
import com.test.java.utilities.LocobuzzLogicalGroupingWorkflowprocess;
...
public class LocobuzzCompetitorsUIComponentsFunctionality {
	...
	@Test
	@Parameters({ "user" })
	public void Competitorsworkflowprocess(String user) throws Exception	 {

		try {
			
			CommonFunctions.login(driver, user);
			
			LocobuzzCompetitorsUIVerificationProcess.goingAccountSession(driver);
			LocobuzzCompetitorsUIVerificationProcess.CompetitorsUIVerificationprocess(driver);
