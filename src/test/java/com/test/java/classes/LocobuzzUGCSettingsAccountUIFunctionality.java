...
import com.test.java.utilities.LocobuzzUGCAccountUIVeriProcess;
...
public class LocobuzzUGCSettingsAccountUIFunctionality {
...
@Test
@Parameters({ "user" })
public void LocoUGCSettingUIVerification(String user) throws Exception	 {

	try {
		
		CommonFunctions.login(driver, user);
		
		LocobuzzUGCAccountUIVeriProcess.goingAccountSession(driver);
		LocobuzzUGCAccountUIVeriProcess.AgentQueueManagementUIVerificationprocess(driver);
