...
public class LocobuzzKeywordConfigRealTimeYoutubeWorkflow {
...
//alert message
			...
			WebElement radioButton = driver.findElement(By.xpath("(//span[contains(text(),' AND ')])[7]"));
			
//adding keywords
			WebElement queryKeyword = driver.findElement(By.xpath("(//input[@placeholder='Add new keyword'])[1]"));
			Actions actions = new Actions(driver);
			Action action = actions.sendKeys(queryKeyword, "videos").sendKeys(Keys.ENTER)
					.sendKeys(queryKeyword, "comments").sendKeys(Keys.ENTER).build();
			action.perform();
			...
			queryKeyword2.sendKeys("no channels added");
...
//search group name
			...
			searchGroupName.sendKeys("youproleg");
