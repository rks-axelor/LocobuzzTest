...
public class LocobuzzKeywordConfigRealTimeInstagramWorkflow {
...
//account session Button
	...
	public static void keywordConfigurationInstagramProcess(WebDriver driver) throws Exception {
		try {
...
//group name
			...
			groupName.sendKeys("ImproKey");
			Thread.sleep(2000);
//select channel
			WebElement selectChannel = driver
					.findElement(By.xpath("(//span[contains(@class,'mat-checkbox-inner-container')])[4]"));
...
//add keywords
			WebElement queryKeyword=driver.findElement(By.xpath("(//input[@placeholder='Add new keyword'])[2]"));
			Actions actions = new Actions(driver);
			Action action = actions.sendKeys(queryKeyword, "newkey").sendKeys(Keys.ENTER)
					.sendKeys(queryKeyword, "views").sendKeys(Keys.ENTER).build();
			action.perform();
...
//search group name
			...
			searchGroupName.sendKeys("ImproKey");
