...
public class LocobuzzKeywordConfigRealTimeNewBlogsWorkflow {
...
//alert messages
			WebElement alertMessages = driver.findElement(By.xpath("//p[@class='alertnote']"));
			if (alertMessages.isDisplayed()) {
				System.out.println("Alert Messages is displayed");
			} else {
				System.out.println("Alert Messages is not displayed");
			}

			WebElement radioButton = driver.findElement(By.xpath("(//span[contains(text(),' AND ')])"));

// adding keywords
			WebElement queryKeyword = driver.findElement(By.xpath("(//input[@placeholder='Add new keyword'])[1]"));
			Actions actions = new Actions(driver);
			Action action = actions.sendKeys(queryKeyword, "price").sendKeys(Keys.ENTER)
					.sendKeys(queryKeyword, "headings").sendKeys(Keys.ENTER).build();
			action.perform();
			
			WebElement queryKeyword1 = driver.findElement(By.xpath("(//input[@placeholder='Add new keyword'])[2]"));
			queryKeyword1.sendKeys("blogs");
			queryKeyword1.sendKeys(Keys.ENTER);
			queryKeyword1.sendKeys("finanace news");
			queryKeyword1.sendKeys(Keys.ENTER);

			WebElement queryKeyword2 = driver.findElement(By.xpath("(//input[@placeholder='Add new keyword'])[3]"));
			queryKeyword2.sendKeys("no.of headings");
...
//search group name
			...
			searchGroupName.sendKeys("NewsKey");
