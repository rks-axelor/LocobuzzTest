...
public class LocoAutoCannedFacebookCSDWorkflow {
...
//ticket information
	public static void ticketTabVerification(WebDriver driver, String channel) throws Exception {
		try {
			...
			ticketInformation.add(0, "Automated Canned Response Escalation  WorkFlow For CSD");
...
// imagename
			WebElement ticketName = driver.findElement(By.xpath("(//span[contains(text(),'Comments')])[1]"));
...
// username
			WebElement userName = driver
					.findElement(By.xpath("//span[contains(text(),'Comments')]//following::a[1]"));
...
// assign
			WebElement assignName = driver
					.findElement(By.xpath("(//mat-icon[contains(text(),'how_to_reg')]//following::span[1])[2]"));
...
// direct close
			WebElement directCloseIcon = driver.findElement(By.xpath("(//mat-icon[contains(text(),'cancel')])[3]"));
...
// opendetails
			...
			if (openDetailsButton.isEnabled() ) {
...
// Automated canned response process
			...
			WebElement selectResponses = driver.findElement(By.xpath("(//span[contains(text(),'HI, what happened!')])[1]"));
			js.executeScript("arguments[0].click()", selectResponses);
			Thread.sleep(5000);

			WebElement selectButton = driver.findElement(By.xpath("(//span[contains(text(),'Select')])[1]"));
...
//Escalate
			...
			if (replyType.trim().equalsIgnoreCase("Reply & Escalate")) {
				...
				WebElement customerName = driver
						.findElement(By.xpath("//span[contains(text(),'komal malicsd')]"));
...
//ticket close 
	...
	//assign
					WebElement assignName=driver.findElement(By.xpath("(//mat-icon[contains(text(),'escalator_warning')]//following::span)[3]"));
					if(assignName.isDisplayed()) {
