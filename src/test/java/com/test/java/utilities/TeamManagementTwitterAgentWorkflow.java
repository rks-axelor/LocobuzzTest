...
public class TeamManagementTwitterAgentWorkflow {
...
//Assigning  to agent
			...
			Thread.sleep(2000);
			selectUser.sendKeys("kalyan agent");
			Thread.sleep(5000);
...
/*	@SuppressWarnings("deprecation")
	public static void agentTicketProcess(WebDriver driver, String channel, String user) throws Exception {
		try {
...
//Click reply post
			WebElement replyPostButton1 = driver.findElement(By.xpath("(//span[contains(text(),'Reply Post')])"));
			JavascriptExecutor js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].click()", replyPostButton1);
			// Actions actions = new Actions(driver);
			// actions.moveToElement(replyPostButton1).click().build().perform();
			// replyPostButton1.click();
			Thread.sleep(2000);
