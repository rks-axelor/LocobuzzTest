...
public class LocobuzzKeywordConfigRealTimeFacebookWorkflow {
...
//account session Button
	...
	public static void keywordConfigurationFacebookProcess(WebDriver driver) throws Exception {
		try {
...
//group name
			...
			groupName.sendKeys("FKey");
			Thread.sleep(2000);
//select channel
			WebElement selectChannel = driver
					.findElement(By.xpath("(//span[contains(@class,'mat-checkbox-inner-container')])[2]"));
			js.executeScript("arguments[0].click()", selectChannel);
			Thread.sleep(3000);
			
//facebook information
			
			WebElement channelInfoHeading=driver.findElement(By.xpath("//div[@class='font-weight-600']"));
			WebElement channelInfo=driver.findElement(By.xpath("//div[@class='text__light text__sm mt-3 line-height-1.5']"));
			WebElement noteMessage=driver.findElement(By.xpath("//p[@class='text__sm font-weight-500']"));
			if(channelInfoHeading.isDisplayed()&&channelInfo.isDisplayed()&&noteMessage.isDisplayed()) {
				System.out.println("Channel Information and note message are displayed");
			}
			else {
				System.out.println("Channel Information and note message are not displayed");
			}
...
//alert note
			WebElement alertNote=driver.findElement(By.xpath("//p[@class='alertnote']"));
			if(alertNote.isDisplayed()) {
				System.out.println("Alert Note is displayed");
			}
			else {
				System.out.println("Alert Note is not displayed");
			}
...
//adding keywords
			...
			Action action = actions.sendKeys(queryKeyword, "priceaction").sendKeys(Keys.ENTER)
			...
			queryKeyword1.sendKeys("Followers");
			queryKeyword1.sendKeys(Keys.ENTER);
			queryKeyword1.sendKeys("views");
			queryKeyword1.sendKeys(Keys.ENTER);
