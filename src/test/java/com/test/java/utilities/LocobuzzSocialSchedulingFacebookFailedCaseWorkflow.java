...
public class LocobuzzSocialSchedulingFacebookFailedCaseWorkflow {
	
	public static List<String> componentsInformation = new ArrayList<String>();
	
	public static void socialSchedulingFacebookFailedCaseWorkflow(WebDriver driver) throws Exception {
		try {
...
//description
			...
			Thread.sleep(3000);
			WebElement emojo = driver.findElement(By.xpath("(//span[contains(@aria-label,'smiley')])[1]"));
...
//failed tab
			...
			WebElement failedTime = driver.findElement(By.xpath("//p[contains(@class,'font-weight-500 ml-5 no-wrap')]"));
			...
			if(postCreatedBy.isDisplayed()&&editButton.isDisplayed()&&chatButton.isDisplayed()&&preview.isDisplayed()&&label.isDisplayed()) {
...
//select profile
			WebElement selectProfile = driver.findElement(By.xpath("//span[contains(text(),'Mynewtestpage')]"));
			js.executeScript("arguments[0].click()", selectProfile);
			Thread.sleep(3000);
...
//partiallyPublished tab
			WebElement partiallyPublishedTab=driver.findElement(By.xpath("(//div[contains(text(),'Partially Published ')])"));
