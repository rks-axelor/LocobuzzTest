...
public class NewMentionTwitterOnholdCreateTicketProcess {
...
// change the status in csd
			csdStatus(driver, ticketIDValue, channel);
			Thread.sleep(2000);
			js.executeScript("arguments[0].click()", search);
			Thread.sleep(2000);
...
// check ticket status
			WebElement ticketStatus = driver.findElement(By.xpath("(//span[@class='post__foot--label'])[4]"));
			if (replyType.trim().equalsIgnoreCase("Reply & Escalate")) {
				if (ticketStatus.getText().trim().contains("OnHold by")) {
...
// open details
				...
				WebElement moreButton = driver.findElement(By.xpath(
						"(//span[@mattooltip='Reply Post'])[2]//following::mat-icon[contains(text(),'more_vert')]"));
				js.executeScript("arguments[0].click()", moreButton);
				WebElement createTicketButton = driver
						.findElement(By.xpath("//span[contains(text(),' Create Ticket')]"));
...
// ticket close
	...
	@SuppressWarnings("deprecation")
	public static void csdStatus(WebDriver driver, String ticketIDValue, String channel) throws Exception {
		try {
...
// status change
			WebElement ticketStatusOpen = driver.findElement(
					By.xpath("(//span[contains(@class,'mat-select-min-line') and contains(.,'Open')])[2]"));
			js.executeScript("arguments[0].click()", ticketStatusOpen);
			WebElement ticketStatusOnhold = driver
					.findElement(By.xpath("//span[@class='mat-option-text' and contains(.,' On Hold ')]"));
			js.executeScript("arguments[0].click()", ticketStatusOnhold);
			Thread.sleep(2000);
			System.out.println("Ticket Status changed to on hold");
...
// Close search
			...
			WebElement onHoldTicketTab = driver.findElement(By.xpath("//a[contains(text(),'On Hold')]"));
			js.executeScript("arguments[0].click()", onHoldTicketTab);
			Thread.sleep(2000);
...
// assign
			 ...
			 * "//mat-icon[contains(text(),'escalator_warning')]//following::span[1]")); if
			 * (assignName.isDisplayed()) { System.out.println("assign name is dispalyed");
			 * } else { System.out.println("assign name is not displayed"); }
			 * 
			 * String assignNameValue = assignName.getText(); ticketInformation.add(16,
			 * assignNameValue); ticketInformation.add(17, ticketStatusValue);
			 */
