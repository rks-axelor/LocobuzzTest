...
public class LocobuzzKeywordConfigRealTimeTeamBHPWorkflow {
...
//account session Button
	...
	public static void keywordConfigurationTeamBHPProcess(WebDriver driver) throws Exception {
		try {
...
//group name
			...
			groupName.sendKeys("proTeam");
			Thread.sleep(2000);
			
//select channel
			WebElement selectChannel = driver
					.findElement(By.xpath("(//span[contains(@class,'mat-checkbox-inner-container')])[9]"));
...
//query builder
			...
			WebElement radioButton = driver.findElement(By.xpath("(//span[contains(text(),'OR ')])[5]"));
...
//adding keywords
			...
			Action action = actions.sendKeys(queryKeyword, "priceaction").sendKeys(Keys.ENTER)
					.sendKeys(queryKeyword, "brandnames").sendKeys(Keys.ENTER).build();
			action.perform();
			...
			queryKeyword2.sendKeys("no comments");
			queryKeyword2.sendKeys(Keys.ENTER);
			
//select country
			WebElement selectCountry = driver.findElement(By.xpath("(//input[@placeholder='Search Countries'])"));
			js.executeScript("arguments[0].click()", selectCountry);
			Thread.sleep(3000);
			selectCountry.sendKeys("india");
			WebElement country = driver.findElement(By.xpath("//span[contains(text(),' India ')]"));
			js.executeScript("arguments[0].click()", country);
			Thread.sleep(3000);
			
			selectCountry.sendKeys("America");
			WebElement country1 = driver.findElement(By.xpath("//span[contains(text(),' United States of America ')]"));
...
//search group name
			...
			searchGroupName.sendKeys("proTeam");
